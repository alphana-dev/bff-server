package ru.icebitsy.bffserver.utils
...
class CookieUtils(
    @Value("\${cookie.domain}")
    private val cookieDomain: String // тот домен, который будет прописываться в сервер-куке при его создании на бекенде
) {

    /**
     * создает server-side cookie со значением jwt. Важно: этот кук сможет считывать только сервер,
    ...
    fun deleteCookie(name: String): HttpCookie {
        return ResponseCookie.from(name, "") // пустое значение
            ...
            .sameSite(SameSiteCookies.NONE.value) // запрет на отправку кука, если запрос пришел со стороннего сайта (доп. защита от CSRF атак) - кук будет отправляться только если пользователь набрал URL в адресной строке
            .httpOnly(true) // кук будет доступен для считывания только на сервере (на клиенте НЕ будет доступен с помощью JavaScript - тем самым защищаемся от XSS атак)
            .secure(true) // кук будет передаваться браузером на backend только если канал будет защищен (https)
            .domain(cookieDomain)
